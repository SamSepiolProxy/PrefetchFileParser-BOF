...
prefetch notepad.exe.pf calc.exe.pf chrome.exe.pf
```
...
- `filters` - (Optional) One or more prefetch filenames to filter results

**Note**: The path argument must contain a backslash (\\) or colon (:) to be recognized as a path. Otherwise, it will be treated as a filter.
